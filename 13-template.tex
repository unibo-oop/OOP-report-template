...
The application will emulate a domotic environment with some smart devices and facilities monitoring.
...
The main difficulties will be implementing a communication protocol between the controller and the devices, and simulating physics of
temperature and air quality along with electrical consumption, in real-time.
...
For the developement of HOMER the MVC pattern will be followed.
This is because the Team wants to clearly seperate the application's business 
logic from the view, and make our software as modular as possibile. 
%Our main building block is the Device interface, 
%and every device will be responsible for its own behaviour.
%INSERT GENERAL ARCHITECTURE UML
\subsubsection{Model}
The Model takes care of data access and modification, thus its main task is going to be providing the application status to the controller.
In this case the application status is represented by the devices' statuses. 
To provide such functionality, the Model of our application is represented by the Device interface, its extensions and implementations.

%INSERT MODEL UML

\subsubsection{View}
%INSERT VIEW DESCRIPTION
%INSERT VIEW UML

\subsubsection{Controller}
The Controller has the task to coordinate the interactions between the user and the View, and requesting the Model to compute them.
Once the Model finishes to elaborate the new state, the Controller will request the View to change accordingly to the new application's state.
To avoid building a "God Class" we chose to split the Controller in a gerarchic structure, encapsulated by the main Controller interface which has
access to the Model and the View.

*ADD LIST OF THE PARTS OF THE CONTROLLER*
%INSERT CONTROLLER UML

By splitting the architecture of the controller we achieved high extensibility possibilities, in fact each pair View-Controller was indipendent from the others.
...
\subsection{Ceccacci Michele}
\subsection{Magnani Simone}
\subsection{Monticelli Alessandro}
\subsubsection{Lights}
\subsubsection{Outlets}
\subsubsection{Electrical Meter}
\subsubsection{Device dashboard}
\subsubsection{Electrical meter dashboard}
